...
async function runScheduledEmailTask(req: Request, res: Response) {
    res.status(200).send("Running scheduled email task");
    scheduledEmail();
}

// Schedule this task to run every day at midnight (00:01)
async function scheduledEmail() {
    ...
    // loop through the reminders and store each event with the same user into the same key
    ...
    for (const username in userMap) {
        ...
        try {
            sendEmail(userMap[username].userEmail, "Events Reminder", message);
        } catch (error) {
            console.error(error);
        }
    }
